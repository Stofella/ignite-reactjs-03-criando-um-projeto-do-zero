...
export default function Home({
  ...
  async function handleFetchMoreClick() {
    ...
    const response = await fetch(nextPageFetchLink).then(response =>
      response.json().then((data: ApiSearchResponse) => data)
    );

    const newPosts: Post[] = response.results.map(post => ({
      uid: post.uid,
      first_publication_date: formatDateLikeRocketseat(
    ...
    <>
      ...
      <div className={`${styles.container} ${commonStyles.contentContainer}`}>
          ...
          <Link href={`/post/${post.uid}`} key={post.uid}>
            <article>
              ...
              <footer>
                <time>
                  <FiCalendar size={20} />
                  {formatDateLikeRocketseat(post.first_publication_date)}
                </time>
                <span>
                  <FiUser size={20} />
                  {post.data.author}
        ...
        {nextPageFetchLink !== undefined && nextPageFetchLink !== null && (
          <p className={styles.loadMore} onClick={handleFetchMoreClick}>
            Carregar mais posts
          </p>
        )}
      </div>
